...
void usage(int ret_code) {
	fprintf(stdout, "usage:  %s [-hd] <bn>\n", progname);
	fprintf(stdout, "Calculate the square of the number 'bn'. The number must be specified as hex,\n");
        fprintf(stdout, "unless you specified the '-d' option to use decimal number.\n");
...
int main(int argc, char *argv[]) {
	...
	if(BN_sqr(sqr, n, ctx) != BN_SUCCESS) {
		ERR_print_errors_fp(stderr);
		goto err;
	}
	char *str = bn2str(sqr);
