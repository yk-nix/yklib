...
void usage(int ret_code) {
	fprintf(stdout, "usage:  %s [-hd] <n> <m>\n", progname);
	fprintf(stdout, "Calculate the value: '(n)mod(m)', and if the value < 0, add it with m.\n");
	fprintf(stdout, "All the numbers must be specified as hexes,\n");
...
int main(int argc, char *argv[]) {
	...
	if(str2bn(&m, argv[optind+1]) == BN_ERROR) {
		ERR_print_errors_fp(stderr);
		goto err;
	}
	if(BN_nnmod(v, n, m, ctx) != BN_SUCCESS) {
		ERR_print_errors_fp(stderr);
		goto err;
	}
	char *str = bn2str(v);
