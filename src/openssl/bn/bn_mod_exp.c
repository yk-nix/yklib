...
void usage(int ret_code) {
	fprintf(stdout, "usage:  %s [-hd] <n> <e> <m>\n", progname);
	fprintf(stdout, "Calculate the value: '(n^e)mod(m)'. All the numbers must be specified as hexes,\n");
...
int main(int argc, char *argv[]) {
	...
	e = BN_CTX_get(ctx);
	m = BN_CTX_get(ctx);
	if(v == NULL || n == NULL || e == NULL || m == NULL) {
	...
	if(BN_mod_exp(v, n, e, m, ctx) != BN_SUCCESS) {
		ERR_print_errors_fp(stderr);
		goto err;
	}
	char *str = bn2str(v);
