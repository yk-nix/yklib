...
void usage(int ret_code) {
	fprintf(stdout, "usage:  %s [-hd] <n1> <n2> <m>\n", progname);
	fprintf(stdout, "Calculate the value: '(n1+n2)mod(m)'. All the numbers must be specified as hexes,\n");
...
int main(int argc, char *argv[]) {
	...
	n2 = BN_CTX_get(ctx);
	m  = BN_CTX_get(ctx);
	if(v == NULL || n1 == NULL || n2 == NULL || m == NULL) {
		ERR_print_errors_fp(stderr);
		goto err;
	}
	if(str2bn(&n1, argv[optind]) == BN_ERROR) {
		ERR_print_errors_fp(stderr);
		goto err;
	}
	if(str2bn(&n2, argv[optind+1]) == BN_ERROR) {
		ERR_print_errors_fp(stderr);
		goto err;
	}
	if(str2bn(&m, argv[optind+2]) == BN_ERROR) {
		ERR_print_errors_fp(stderr);
		goto err;
	}
	if(BN_mod_add(v, n1, n2, m, ctx) != BN_SUCCESS) {
		ERR_print_errors_fp(stderr);
		goto err;
	}
	char *str = bn2str(v);
