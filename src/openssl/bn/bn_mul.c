...
void usage(int ret_code) {
	fprintf(stdout, "usage:  %s [-hd] <bn1> <bn2> [<bn3>...]\n", progname);
	fprintf(stdout, "Calculate the products of the numbers. All the numbers must be specified as hexes,\n");
...
int main(int argc, char *argv[]) {
	...
	BN_CTX_start(ctx);
	BIGNUM *product, *n;
	product = BN_CTX_get(ctx);
	n   = BN_CTX_get(ctx);
	str2bn(&product, "1");
	for(i = optind; i < argc && argv[i]; i++) {
		if(str2bn(&n, argv[i]) == BN_ERROR) {
			ERR_print_errors_fp(stderr);
			goto err;
		}
		if(BN_mul(product, product, n, ctx) != BN_SUCCESS) {
			ERR_print_errors_fp(stderr);
			goto err;
		}
	}
	char *str = bn2str(product);
	if(str) {
		fprintf(stdout, "%s\n", str);
		OPENSSL_free(str);
		ret = 0;
