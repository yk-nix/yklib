...
#define SYSCONF_NUM_MAX		255

#define CMD_UNKOWN              0
#define CMD_LIST		1

static int cmd = CMD_UNKOWN;
void init(int argc, char *argv[])
{
	...
	while((opt = getopt_long(argc, argv, "l", options, &optidx)) != -1){
		switch(opt){
		case 'l':
			cmd = CMD_LIST;
			break;
...
static struct sysconf_entry sysconf_table[SYSCONF_NUM_MAX] = {
	{"_SC_ARG_MAX", _SC_ARG_MAX},
	{"_SC_CHILD_MAX", _SC_CHILD_MAX},
	{"_SC_CLK_TCK", _SC_CLK_TCK},
	{"_SC_NGROUPS_MX", _SC_NGROUPS_MAX},
...
void cmd_list()
{
	struct sysconf_entry *entry;
	int i = 0;
	for(entry = &sysconf_table[i]; entry && entry->name && i < SYSCONF_NUM_MAX; 
		entry = &sysconf_table[++i]){
		
		printf("%03d	%-30s	%d\n", entry->id, entry->name, sysconf(entry->id));
	}
}
